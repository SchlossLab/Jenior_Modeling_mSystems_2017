---
title: *Clostridium difficile* colonizes alternative nutrient niches during infection across distinct murine gut environments
csl: nature.csl
output: pdf
...
setwd('~/Desktop/Jenior_Transcriptomics_2015/')

# Point to data files
# Table 1
antibiotic_file <- 'data/abx.tsv'
# Figure 2
cfu_file <- 'data/wetlab_assays/cfu.dat'
toxin_file <- 'data/wetlab_assays/toxin_titer.dat'
# Figure 5
network_file <- 'data/metabolic_models/cefoperazone_630.bipartite.files/bipartite_graph.tsv'
# Figure 6
growth_file <- 'data/wetlab_assays/cd630_growth.tsv'
# Figure S2
acetate_file <- 'data/wetlab_assays/cef_acetate_630.txt'

# Set seed to the answer to the ultimate question of life, the universe, and everything
set.seed(42)
...
tox_p_values <- c(gf_vs_cef_tox_p,gf_vs_clinda_tox_p,gf_vs_strep_tox_p,cef_vs_clinda_tox_p,cef_vs_strep_tox_p,clinda_vs_strep_tox_p)
tox_p_values <- round(p.adjust(tox_p_values, method='holm'), digits=3)
gf_vs_cef_tox_cp <- tox_p_values[1]
gf_vs_clinda_tox_cp <- tox_p_values[2]
gf_vs_strep_tox_cp <- tox_p_values[3]
cef_vs_clinda_tox_cp <- tox_p_values[4]
cef_vs_strep_tox_cp <- tox_p_values[5]
clinda_vs_strep_tox_cp <- tox_p_values[6]
...
gf_vs_cef_spore_p <- wilcox.test(germfree, cefoperazone, exact=F)$p.value
gf_vs_clinda_spore_p <- wilcox.test(germfree, clindamycin, exact=F)$p.value
gf_vs_strep_spore_p <- wilcox.test(germfree, streptomycin, exact=F)$p.value
clinda_vs_cef_spore_p <- wilcox.test(clindamycin, cefoperazone, exact=F)$p.value
strep_vs_cef_spore_p <- wilcox.test(streptomycin, cefoperazone, exact=F)$p.value
strep_vs_clinda_spore_p <- wilcox.test(streptomycin, clindamycin, exact=F)$p.value
spore_p_values <- c(gf_vs_cef_spore_p,gf_vs_clinda_spore_p,gf_vs_strep_spore_p,clinda_vs_cef_spore_p,strep_vs_cef_spore_p, strep_vs_clinda_spore_p)
spore_p_values <- round(p.adjust(spore_p_values, method='holm'), digits=3)
gf_vs_cef_spore_cp <- spore_p_values[1]
gf_vs_clinda_spore_cp <- spore_p_values[2]
gf_vs_strep_spore_cp <- spore_p_values[3]
clinda_vs_cef_spore_cp <- spore_p_values[4]
strep_vs_cef_spore_cp <- spore_p_values[5]
strep_vs_clinda_spore_cp <- spore_p_values[6]
...
# Get values of interest for text
...
To validate the biological relevance of substrates identified as uniquely important to *C. difficile* metabolism through our network-based analysis, we tested whether *C. difficile* was able to utilize each substrate for *in vitro* growth (Fig. 6c). This was performed using the modified defined *C. difficile* minimal media, supplemented individually with the selected carbohydrates (Theriot, 2013) implicated by high importance scores. Because *C. difficile* can use amino acids for growth through Stickland fermentation, but is auxotrophic for several amino acids. As such, the most effective negative control is growth in media lacking carbohydrates but containing amino acids (Max OD600 = 0.212). 
  As previously stated, N-acetyl-D-glucosamine was shown to be important to *C. difficile* in each condition tested. When tested for improved growth, significantly more growth (Max OD600 = 0.774) was observed compared to no carbohydrate controls (P << 0.001). This provided evidence that N-acetyl-D-glucosamine, derived from the host mucus layer, may be a central component of the *C. difficile* nutritional niche during infection. Furthermore, at least one carbohydrate highlighted as distinctly more important in each of the antibiotic treatment groups provided high levels of *C. difficile* growth relative to control wells (P << 0.001). This included D-fructose (streptomycin; Max OD600 = 0.671), mannitol (cefoperazone; Max OD600 = 0.464), salicin (clindamycin; Max OD600 = 0.869), and N-acetylneuraminate (GF; Max OD600 = 0.439). Because it was not possible to test aminofructose directly, we instead chose to test fructose, an immediate breakdown byproduct of aminofructose catabolism. We also tested both starch and acetate for the ability to support *C. difficile* growth *in vitro*, but neither should any improvement over No Carbohydrate control (Fig. S3). Maximum growth rate analysis for each carbohydrate also indicates potential hierarchy of growth nutrient preference (Table S4). The progression is as follows: D-fructose (slope = 0.089), N-acetyl-D-glucosamine (slope = 0.085), salicin (slope = 0.077), mannitol (slope = 0.044), and finally N-acetylneuriminate (slope = 0.024). This suggests that *C. difficile* is most well-suited to metabolize the nutrient source that is most likely to be present in all susceptible mammalian GI tracts. Together, these results support the hypothesis that *C. difficile* is able to adapt its nutritional strategy to carbohydrate availability during infection and may suggest a hierarchy of nutrient preference.
...
trehalose_p <- summary(aov(formula=od ~ substrate * time, data=trehalose_test))[[1]][["Pr(>F)"]][3]
starch_p <- summary(aov(formula=od ~ substrate * time, data=starch_test))[[1]][["Pr(>F)"]][3]
fructose_p <- summary(aov(formula=od ~ substrate * time, data=fructose_test))[[1]][["Pr(>F)"]][3]
...
aov_p_values <- c(trehalose_p,starch_p,fructose_p,mannitol_p,salicin_p,acetate_p,acetylglucosamine_p,acetylneuraminate_p,bhi_p,no_aa_p)
aov_p_values <- round(p.adjust(aov_p_values, method='holm'), digits=3)
trehalose_cp <- aov_p_values[1]
starch_cp <- aov_p_values[2]
fructose_cp <- aov_p_values[3]
mannitol_cp <- aov_p_values[4]
salicin_cp <- aov_p_values[5]
acetate_cp <- aov_p_values[6]
acetylglucosamine_cp <- aov_p_values[7]
acetylneuraminate_cp <- aov_p_values[8]
bhi_cp <- aov_p_values[9]
no_aa_cp <- aov_p_values[10]
...
# Maximum growth rate
acetate_max_rate <- round(diff(acetate_median)[which.max(diff(acetate_median))], digits=3)
...
trehalose_max_rate <- round(diff(trehalose_median)[which.max(diff(trehalose_median))], digits=3)
fructose_max_rate <- round(diff(fructose_median)[which.max(diff(fructose_median))], digits=3)
...
acetate_time_max_rate <- round((which.max(diff(acetate_median)) * 0.5), digits=3) - 0.5
acetylglucosamine_time_max_rate <- round((which.max(diff(acetylglucosamine_median)) * 0.5), digits=3) - 0.5
acetylneuraminate_time_max_rate <- round((which.max(diff(acetylneuraminate_median)) * 0.5), digits=3) - 0.5
trehalose_time_max_rate <- round((which.max(diff(trehalose_median)) * 0.5), digits=3) - 0.5
fructose_time_max_rate <- round((which.max(diff(fructose_median)) * 0.5), digits=3) - 0.5
mannitol_time_max_rate <- round((which.max(diff(mannitol_median)) * 0.5), digits=3) - 0.5
salicin_time_max_rate <- round((which.max(diff(salicin_median)) * 0.5), digits=3) - 0.5
bhi_time_max_rate <- round((which.max(diff(bhi_median)) * 0.5), digits=3) - 0.5
no_aa_time_max_rate <- round((which.max(diff(no_aa_median)) * 0.5), digits=3) - 0.5
no_carb_time_max_rate <- round((which.max(diff(no_carb_median)) * 0.5), digits=3) - 0.5

# Maximum OD
max_od <- round(max(acetate_median), digits=3)
max_od <- round(max(acetylglucosamine_median), digits=3)
max_od <- round(max(acetylneuraminate_median), digits=3)
max_od <- round(max(trehalose_median), digits=3)
max_od <- round(max(fructose_median), digits=3)
max_od <- round(max(mannitol_median), digits=3)
max_od <- round(max(salicin_median), digits=3)
max_od <- round(max(bhi_median), digits=3)
max_od <- round(max(no_aa_median), digits=3)
max_od <- round(max(no_carb_median), digits=3)

# Time of max OD
acetate_time_max_od <- round((which.max(acetate_median) * 0.5), digits=3) - 0.5
acetylglucosamine_time_max_od <- round((which.max(acetylglucosamine_median) * 0.5), digits=3) - 0.5
acetylneuraminate_time_max_od <- round((which.max(acetylneuraminate_median) * 0.5), digits=3) - 0.5
trehalose_time_max_od <- round((which.max(trehalose_median) * 0.5), digits=3) - 0.5
fructose_time_max_od <- round((which.max(fructose_median) * 0.5), digits=3) - 0.5
mannitol_time_max_od <- round((which.max(mannitol_median) * 0.5), digits=3) - 0.5
salicin_time_max_od <- round((which.max(salicin_median) * 0.5), digits=3) - 0.5
bhi_time_max_od <- round((which.max(bhi_median) * 0.5), digits=3) - 0.5
no_aa_time_max_od <- round((which.max(no_aa_median) * 0.5), digits=3) - 0.5
no_carb_time_max_od <- round((which.max(no_carb_median) * 0.5), digits=3) - 0.5

# Growth rate at 24 hours
acetate_rate_24_hrs <- round(diff(acetate_median)[length(diff(acetate_median))], digits=3)
acetylglucosamine_rate_24_hrs <- round(diff(acetylglucosamine_median)[length(diff(acetylglucosamine_median))], digits=3)
acetylneuraminate_rate_24_hrs <- round(diff(acetylneuraminate_median)[length(diff(acetylneuraminate_median))], digits=3)
trehalose_rate_24_hrs <- round(diff(trehalose_median)[length(diff(trehalose_median))], digits=3)
fructose_rate_24_hrs <- round(diff(fructose_median)[length(diff(fructose_median))], digits=3)
mannitol_rate_24_hrs <- round(diff(mannitol_median)[length(diff(mannitol_median))], digits=3)
salicin_rate_24_hrs <- round(diff(salicin_median)[length(diff(salicin_median))], digits=3)
bhi_rate_24_hrs <- round(diff(bhi_median)[length(diff(bhi_median))], digits=3)
no_aa_rate_24_hrs <- round(diff(no_aa_median)[length(diff(no_aa_median))], digits=3)
no_carb_rate_24_hrs <- round(diff(no_carb_median)[length(diff(no_carb_median))], digits=3)

# Mean growth rate
acetate_mean_rate <- round(mean(diff(acetate_median)), digits=3)
acetylglucosamine_mean_rate <- round(mean(diff(acetylglucosamine_median)), digits=3)
acetylneuraminate_mean_rate <- round(mean(diff(acetylneuraminate_median)), digits=3)
trehalose_mean_rate <- round(mean(diff(trehalose_median)), digits=3)
fructose_mean_rate <- round(mean(diff(fructose_median)), digits=3)
mannitol_mean_rate <- round(mean(diff(mannitol_median)), digits=3)
salicin_mean_rate <- round(mean(diff(salicin_median)), digits=3)
bhi_mean_rate <- round(mean(diff(bhi_median)), digits=3)
no_aa_mean_rate <- round(mean(diff(no_aa_median)), digits=3)
no_carb_mean_rate <- round(mean(diff(no_carb_median)), digits=3)
...
***C. difficile* cultivation and quantification.** Differential plating was performed to quantify both *C. difficile* spores and vegetative cells respectively. Cecal samples were weighed and serially diluted under anaerobic conditions (6% H, 20% CO2, 74% N2) with anaerobic PBS. Samples were plated on CCFAE plates (fructose agar plus cycloserine, cefoxitin, and erythromycin) and counted for C.f.u. after 24 hours of anaerobic growth at 37° C (Buggy, 1997). It is important to note that the germination agent taurocholate was omitted from these plates in order to only quantify vegetative cells. Undiluted samples with then heated at 60° C for 30 minutes in order to eliminate vegetative cells and leave only spores (Sorg, 2001). These samples were then serially diluted under anaerobic conditions in anaerobic PBS and plated on CCFAE + taurocholate. Plating was simultaneously done for heated samples on CCFAE to ensure all vegetative cells had been eliminated. After 24 hours of anaerobic incubation at 37° C, c.f.u.s were quantified.
