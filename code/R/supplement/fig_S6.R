...
isoleucine_mock <- mock_metabolome[, c(1,which(colnames(mock_metabolome) %in% c('isoleucine')))]
colnames(isoleucine_mock) <- c('abx', 'substrate')
isoleucine_mock$abx <- factor(isoleucine_mock$abx, levels=c('none','streptomycin','cefoperazone','clindamycin','germfree'))
leucine_mock <- mock_metabolome[, c(1,which(colnames(mock_metabolome) %in% c('leucine')))]
colnames(leucine_mock) <- c('abx', 'substrate')
leucine_mock$abx <- factor(leucine_mock$abx, levels=c('none','streptomycin','cefoperazone','clindamycin','germfree'))
...
p.adjust(c(wilcox.test(subset(isoleucine_mock, abx=='none')[,2], subset(isoleucine_mock, abx=='streptomycin')[,2], exact=F)$p.value,
           wilcox.test(subset(isoleucine_mock, abx=='none')[,2], subset(isoleucine_mock, abx=='cefoperazone')[,2], exact=F)$p.value,
           wilcox.test(subset(isoleucine_mock, abx=='none')[,2], subset(isoleucine_mock, abx=='clindamycin')[,2], exact=F)$p.value,
           wilcox.test(subset(isoleucine_mock, abx=='none')[,2], subset(isoleucine_mock, abx=='germfree')[,2], exact=F)$p.value), method='BH')
p.adjust(c(wilcox.test(subset(leucine_mock, abx=='none')[,2], subset(leucine_mock, abx=='streptomycin')[,2], exact=F)$p.value,
           wilcox.test(subset(leucine_mock, abx=='none')[,2], subset(leucine_mock, abx=='cefoperazone')[,2], exact=F)$p.value,
           wilcox.test(subset(leucine_mock, abx=='none')[,2], subset(leucine_mock, abx=='clindamycin')[,2], exact=F)$p.value,
           wilcox.test(subset(leucine_mock, abx=='none')[,2], subset(leucine_mock, abx=='germfree')[,2], exact=F)$p.value), method='BH')
...
isoleucine_mock <- subset(isoleucine_mock, abx == 'none')
leucine_mock <- subset(leucine_mock, abx == 'none')
...
# Isoleucine
stripchart(substrate~abx, data=isoleucine_mock, vertical=T, pch=19, 
...
# Leucine
stripchart(substrate~abx, data=leucine_mock, vertical=T, pch=19, 
           xaxt='n', yaxt='n', col='gray40', ylim=c(0,5), xlim=c(0.5,13.5),
           cex=1.2, ylab='Scaled Intesity', method='jitter', jitter=0.15)
