...
**Supplementary Table 1 | Specific genes and normalized cDNA read abundances included in analysis reported in Figure 2.** Transcript abundances reported in each of the antibiotic associated columns were first normalized to both sequencing read length and target gene length. This yielded a per base coverage value for each gene of interest. Each of the three groups were then even subsampled to an equal total sequences abundance of 13,000 reads to allow for comparability between groups. Additional columns indicate specific gene annotation (gene, pathways, & KEDD_ID) as well as which group each gene belongs for ternary plot (family).


**Supplementary Table 2 | Normalized cDNA read abundances, gene annotations, and enzymatic reaction information used for metabolic model building for *C. difficile* str. 630 KEGG orthologs across colonized conditions.** The 




**Supplementary Table 3 | Topology metrics for enzyme and metabolite nodes in the *C. difficile* str. 630 metabolic network.**

**Supplementary Table 4 | Metabolites with significant importance scores for *C. difficile* in each colonized condition.**

**Supplementary Table 5 | Growth curve analysis for each carbon source.**

**Supplementary Table 6 | Transcriptomic read counts at each step of curation.**
